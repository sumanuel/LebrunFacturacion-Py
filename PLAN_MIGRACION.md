...
- `clases/` (equivalente a clases/ original):
  - `bancos/`, `clientes/`, `complementos/`, `contabilidad/`, `facturacion/`, `vendedores/`: Carpetas para clases de negocio.
- `formularios/` (equivalente a formularios/ original):
  - `administracion/`, `bancos/`, `clientes/`, `complementos/`, `facturacion/`, `vendedores/`: Carpetas para UI.
- `config.py`: Configuraciones globales.
- `Resources/`: Imágenes.
- Otros archivos: README.md, requirements.txt, PLAN_MIGRACION.md.
...
- `FuncionesTexbox.cs` → **Completado:** Funciones de validación agregadas en `utils.py`.
- `Inputbox.cs` → **Completado:** Diálogo de entrada simple en `utils.py` (mejorable con Tkinter).
...
- `facturacion/` → **Completado:** Clases `Factura` y `FacturaFerle` en `clases/facturacion/factura.py` y `factura_ferle.py` con métodos básicos de facturación e impresión.
...
**Estado Actual:** Fase 2 completada. Todas las clases de negocio migradas. Listo para Fase 3 (UI).
