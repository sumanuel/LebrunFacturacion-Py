...
**Archivos a Migrar:**
- `ConexionBD.cs` → Mejorar `database.py` con pools de conexiones, transacciones y configuración robusta.
- `globales.cs` → Fusionar en `config.py` como constantes globales.
- `UsuarioSistema.cs` → Crear clase `Usuario` en Python con métodos de autenticación.
...
**Estado Actual:** Fase 1 pendiente de iniciar.